# 종목 누적 분석 프로젝트 지시문 v4.7.0

상태: **SPEC FROZEN**  
직전 기준: `v4.6-candidate.4 / SPEC CANDIDATE`  
동결 시각: `2026-08-12T00:34+09:00`  
...
2. 기존 Wiki 종목이 오래된 상태인 채 신규 후보와 경쟁하거나 최종 추천에 남는 것을 금지한다.
3. 의미 있는 판단 변화가 있으면 Current View + Revision Timeline + INDEX를 갱신한다.
4. 상태가 그대로인 단순 반복확인은 새 Revision을 만들지 않되, 필요하면 최근 검증세션 메타데이터만 갱신할 수 있다.
5. 최신 완료장으로 재검증하지 못한 기존 종목은 과거의 `매입 대기`·`선발` 상태를 그대로 현재 추천 근거로 사용하지 않고 `자료 제한/자료 부족`으로 취급한다.
6. 기존 Wiki 종목 재검증 → 신규 후보 탐색 → 기존+신규 동일기준 비교 → 국가별 최대 1종목 선정 순서를 고정한다.
7. LAB `active_experiment_hashes`의 canonical 형식은 experiment_hash 문자열 목록이며 setup별 대응은 `active_experiment_map`으로 분리한다.
...
기존 Wiki 종목이면 기존 페이지를 먼저 읽고 최신 완료장과 비교한다.
...
5. 의미 있는 변화가 있으면 Wiki 저장
6. 신규 후보 탐색
7. 기존 Wiki 재검증 종목과 신규 후보를 **동일한 최신 완료장 기준**으로 비교
8. 국가별 최대 1종목 선정
9. 조건 미달이면 `추천 없음`

활성 Wiki 종목은 원칙적으로 INDEX에 등재되어 있고 `해제`가 아닌 종목을 뜻한다. `자료 부족` 종목도 과거 상태가 현재 추천에 영향을 줄 수 있으므로 필요 시 재검증 대상에 포함한다.

기존 종목을 재검증하지 못했다면 그 종목의 과거 `즉시 매입 가능 / 매입 대기 / 선발 / 관찰 / 보유` 상태를 현재 추천 근거로 사용하지 않는다.
...
핵심자료가 부족하면 `자료 제한` 또는 `자료 부족`으로 표시한다.
...
2. 현재 동결 지시문 `chatgpt/INSTRUCTION_v4.7.0.md`
...
4. 시장상태를 먼저 재판정한다.
5. 기업위험 → 종목구조 → 가격·거래량 → 진입상태 순으로 재검증한다.
6. 기존 상태가 유지되는지, 승격되는지, 하향되는지, 해제되는지 판정한다.
7. 재검증 완료 전 신규 후보와 비교하지 않는다.
8. 재검증할 수 없는 종목은 `자료 제한/자료 부족`으로 처리하고 과거의 매입 가능 상태를 현재 추천 근거로 사용하지 않는다.
9. 그 후에만 신규 후보를 탐색한다.
10. 재검증된 기존 종목과 신규 후보를 동일 기준으로 비교한다.
...
`v4.6-candidate.4 / SPEC CANDIDATE / SUPERSEDED HISTORY`
...
26. **기존 Wiki 종목의 완료세션이 stale인데 과거 상태를 현재 추천에 재사용하지 않았는가**
27. **기존 종목과 신규 후보를 같은 시장·같은 완료세션 기준으로 비교했는가**
28. **상태가 바뀐 기존 종목의 Current View·Revision·INDEX를 함께 동기화했는가**
29. **상태가 유지된 단순 재확인을 새 판단 Revision으로 남발하지 않았는가**
30. **기존 Wiki 종목 재검증 결과를 전체 추천 출력에 표시했는가**
31. `active_experiment_hashes`가 문자열 목록이고 setup별 대응이 `active_experiment_map`으로 분리되었는가
...
- Previous: `v4.6-candidate.4`
- 핵심 동결사항: **전체 추천 질의에서 기존 활성 Stock Wiki 종목 선재검증을 의무화하고, stale 상태를 현재 추천에 재사용하지 않는다.**

이 파일 자체는 수정하지 않는다. 변경이 필요하면 `v4.7.1` 또는 다음 새 버전으로 생성하고, 기존 동결본은 보존한다.
